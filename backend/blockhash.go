...
func (backend *Backend) CacheRecentBlockHash() {
	...
	for _, x := range backend.clientBH {
		rpcClients = append(rpcClients, rpc.New(x))
	}
	index := 0
	for {
		select {
		case <-backend.updateBH:
			/*
			...
			*/
			...
			if backend.cachedBH[2] == getRecentBlockHashResult.Value.Blockhash {
				continue
			}
			for !atomic.CompareAndSwapInt32(&backend.lock, 0, 1) {
				continue
			}
			backend.cachedBH = append(backend.cachedBH, getRecentBlockHashResult.Value.Blockhash)
